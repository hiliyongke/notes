...
组合继承也叫伪经典继承，综合了原型链和[盗用构造]函数，将两者有点结合了起来。基本的思路就是使用原型链继承原型上的属性和方法，而通过盗用构造函数继承实例属性。这样可以把方法定义在原型上以实现重用，又可以让实例有自己的属性。
...
但是后来出现了 [Object.create()] 方法，将 `原型式继承` 的概念规范化了。
