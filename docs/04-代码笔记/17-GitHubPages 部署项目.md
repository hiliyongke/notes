...
4. 解析线路默认
5. 记录值为 `你的 Github 用户名 + github.io`，比如我就填写 `hiliyongke.github.io`
6. TTL 默认
...
在项目的 `public` 目录下，新建 `CNAME` 文件，没有后缀名，里面填写将来要部署的地址位置，我在之前二级域名添加了 `demo` 的二级域名，那么我的 `CNAME` 文件就填写 `dem.xx.cn`。
